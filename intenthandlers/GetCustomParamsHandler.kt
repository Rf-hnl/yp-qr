...
class GetCustomParamsHandler(private val activity: Activity) {
    ...
    fun handle() {
        val result = Intent("icg.actions.electronicpayment.rfhnlyp.GET_CUSTOM_PARAMS").apply {
            putExtra("Name", "Pago Yappy QR")

            // Cargar logo desde recursos (res/drawable) y convertirlo a Base64
            val logoBytes = getLogoBase64("yappy_logo") // sin extensión
            if (logoBytes != null) {
